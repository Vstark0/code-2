...
const calculator = (num1, num2, callback) => {
    //line below checks if the variables can be converted into numbers, then we code to do the actual converting
  if(+num1 && +num2){
    num1 = +num1;
    num2 = +num2;
...
//add is the callback on line 23
...
applyDiscountsByCategory(catProducts, 2, applyPercentDiscount, .3);
console.log(catProducts);
...
const copyStrToSnakeCase = str => {
    str = str.toLowerCase();
    const splitStr = str.split(' ');
    const snakeCaseStr = splitStr.join(' ');
    return snakeCaseStr;
}

const lotrCamel = copyArrAndChange(lotr, copyStrToCamelCase);
const lotrSnake = copyArrAndChange(lotr, copyAtrToSnakeCase);
...
//uncomment line below before starting problems
